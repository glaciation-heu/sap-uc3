...
impl AmphoraApi {
    ...
    async fn post_masked_input(
        ...
        event!(Level::DEBUG, "Adding masked input. Data lenght: {}", body.0.data.len());
...
static P: Lazy<BigInt> = Lazy::new(|| BigInt::from_str("198766463529478683931867765928436695041").unwrap());
static R: Lazy<BigInt> = Lazy::new(|| BigInt::from_str("141515903391459779531506841503331516415").unwrap());
static R_INV: Lazy<BigInt> = Lazy::new(|| BigInt::from_str("133854242216446749056083838363708373830").unwrap());
...
impl InputMasksObject {
    ...
    fn generate_for(s: (BigInt, BigInt)) -> InputMasksObject {
        ...
        let w = (
            s.0.clone().mul(&r.0).add(s.0.clone().mul(&r.1)).mod_floor(&P), //w_0 = s_0 * r_0 + s_0 * r_1
            s.1.clone().mul(&r.1).add(s.1.clone().mul(&r.0)).mod_floor(&P), //w_1 = s_1 * r_1 + s_1 * r_0
        );
        let u = (
            v.0.clone().mul(&r.0).add(v.0.clone().mul(&r.1)).mod_floor(&P), // u_0 = v_0 * r_0 + v_0 * r_1
            v.1.clone().mul(&r.1).add(v.1.clone().mul(&r.0)).mod_floor(&P), // u_1 = v_1 * r_1 + v_1 * r_0
        );
        InputMasksObject {
            secret: s,
            r,
...
fn generate_input_masks(vcp: i32, _id: String, count: i64) -> OutputDeliveryObject {
    // It is ok to just generate a new input-mask object for each share, 
    // because no randomness is used.
    let mut input_masks: Vec<InputMasksObject> = Vec::new();
    for _ in 0..count {
        input_masks.push(InputMasksObject::generate());
    }
    let bin_output_delivery;
    if vcp > 0 {
        bin_output_delivery = input_masks.iter().rfold(
            (
                Vec::<u8>::new(),
                Vec::<u8>::new(),
                Vec::<u8>::new(),
...
fn get_secret_share(vcp: i32, secret_id: &String, _request_id: &String) -> OutputDeliveryObject {
    let secrets = GLOBAL_SECRETS.lock().unwrap();
    if let Some(secret) = secrets.get(secret_id) {
        event!(Level::INFO, "get shares vcp {}, s1: {}, s2: {}", vcp, secret[0].0, secret[0].1);
        let mut input_masks: Vec<InputMasksObject> = Vec::new();
        for s in secret {
            input_masks.push(InputMasksObject::generate_for(s.clone()));
        }

        let bin_output_delivery = secret.iter()
        .map(|v| {
            if vcp > 0 {
                &v.1
            } else {
...
fn add_secrets(vcp: i32, secret_id: &String, data: &Vec<DataObject>) {
    ...
    let decoded_data = data.iter().map(|d| BASE64_STANDARD.decode(&d.value).expect("Unable to decode base64 encoded data")).map(|decoded| to_bigint_arr(&decoded, &R_INV, &P)).flatten().collect::<Vec<BigInt>>();
    let mut input_masks: Vec<InputMasksObject> = Vec::new();
    for _ in &decoded_data {
        input_masks.push(InputMasksObject::generate());
    }
    let shares: Vec<BigInt> = decoded_data.iter().zip(input_masks).map(|z| {
        let secret_to_add: BigInt = if vcp > 0 {
            z.1.secret.1.clone()
            } else {
                z.1.secret.0.clone()
        };
        event!(Level::INFO, "vcp {} Adding {} to {}", vcp, z.0, secret_to_add);
        let mut val = z.0.add(
            secret_to_add.mul(BigInt::from_str("2").unwrap())
    ...
    if secrets.contains_key(secret_id) {
        secrets.get_mut(secret_id).unwrap().iter_mut().zip(shares).for_each(|(s,v)| {
                if vcp > 0 {
                    s.1 = v;
                } else {
                    s.0 = v;
        ...
        secrets.insert(secret_id.clone(), shares.iter().map(|s| {
            if vcp > 0 {
...
mod test {
    ...
    fn test_secret() {
        ...
        let s_0 = to_bigint_arr(&BASE64_STANDARD.decode(odo_0.secret_shares).unwrap(), &R_INV, &P);
        let s_1 = to_bigint_arr(&BASE64_STANDARD.decode(odo_1.secret_shares).unwrap(), &R_INV, &P);
    ...
    fn complete_test() {
        ...
        let secret = BigInt::from_str("25").unwrap();
        ...
        let share_list = vec![DataObject { value: encoded_share}];
        // Add secrets
